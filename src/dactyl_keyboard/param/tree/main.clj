...
(def raws
  ...
  [["# General configuration options\n\n"
   ...
   [:parameter [:main-body :tweaks]
    ...
    "Their only technical significance lies in the fact that "
    ...
    "and places a cuboid shape there. The elements of a leaf are, in order:\n"
    "\n"
    "1. Mandatory: An anchor. This is the name of a feature, such as a key "
    "by its `alias`.\n"
    "2. Optional: A compass point code, such as `SW` for south-west or "
    "`NNE` for north by north-east, identifying a side of the anchor. "
    "There is no default value. If this code is omitted, i.e. if only the "
    "mandatory element is given, the tweak will use the middle of the named "
    "feature.\n"
    "3. Optional: A starting vertical segment ID, which is an integer from 0 "
    "up to a maximum number that varies with the type of the anchor. "
    "Again there is no default.\n"
    "4. Optional: A stopping wall segment ID. If this is provided, it must be "
    "at least as great as the starting segment ID, in which case the leaf "
    "will represent the convex hull of the two indicated segments plus all "
    "segments between them, off the same anchor.\n"
